 ...
 */
public class BinFileMultColRam implements BinFileMultCol
{
	/**
	 ...
	 */
	public BinFileMultColRam(String fileName, int columns)
	{
		if (columns < 2)
		{
		...
		if (existsFileDump(fileName))
		{
			...
			this.dataFile = new byte[ (int) (new File(fileName + BinFileMultColPreparer.ext1)).length()];
		}
		else
		{
		...
		try
		{
			...
			if (fileName != null)
			{
				...
				reader2 = new RandomAccessFile(fileName + BinFileMultColPreparer.ext1, "r");
			}

			reader1.read(this.indexFile);
			reader1.close();
			reader2.read(this.dataFile);
			reader2.close();
		}
		catch (Exception ex)
		{
	...
	public List getData(Integer number, int myColumns, int maxItems)
	{
		...
		try
		{
			...
			for (int i = fromPos; i < toPos && realCount < maxItems; i++)
			{
				...
				{
					values[j] = Converter.bytesToInteger(this.dataFile, i*((this.columns-1)*4)+j*4);
				}
				retVec.add(values);
				realCount++;
			}
		...
		try
		{
			...
			for (int i = fromPos; i < toPos && realCount < maxItems; i++)
			{
				...
				for (int j = 0; j < (this.columns - 1); j++)
				{
					...
					{
						values[j] = Converter.bytesToInteger(this.dataFile, i*((this.columns-1)*4) +j*4);
						if (values[j].intValue() < minCols[j+1] || values[j].intValue() > maxCols[j+1])
						{
							add = false;
						}
					}
	...
	public Map getDataAsMap(Integer number, int myColumns, int maxItems)
	{
		...
		try
		{
			...
			for (int i = fromPos; i < toPos && realCount < maxItems; i++)
			{
				// read all values of a row ( assumedly there are this.columns columns,
				// we need to read this.columns - 1
				Integer key = Converter.bytesToInteger(this.dataFile, i*((this.columns-1)*4) );
				Integer[] values = new Integer[myColumns - 2];
				for (int j = 1; j < (this.columns - 1); j++)
				{
					values[j-1] = Converter.bytesToInteger(this.dataFile, i*((this.columns-1)*4) +j*4);
				}
		...
		try
		{
			...
			for (int i = fromPos; i < toPos && realCount < maxItems; i++)
			{
				...
				for (int j = 1; j < (this.columns - 1); j++)
				{
					...
					{
						values[j-1] = Converter.bytesToInteger(this.dataFile, i*((this.columns-1)*4) +j*4);
						if (values[j-1].intValue() < minCols[j+1] || values[j-1].intValue() > maxCols[j+1])
						{
							add = false;
						}
					}
	...
	public List getSingleColumn(Integer number, int maxItems, int col)
	{
		...
		try
		{
			...
			for (int i = fromPos; i < toPos && realCount < maxItems; i++)
			{
				// read all values of a row ( assumedly there are this.columns columns,
				// we need to read this.columns - 1
				value = Converter.bytesToInteger(this.dataFile, i*((this.columns-1)*4)+col*4);
				retVec.add(value);
				realCount++;
			}
		}
	...
	public static void main(String[] args)
	{
		...
		else
		{
			...
			BinFileMultCol prep = new BinFileMultColRam(args[0], columns.intValue());

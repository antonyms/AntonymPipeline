...
public class FuncToSparseVector extends Function {
	...
	private void putElement(ATerm t) {
		ScanTerm pair = (ScanTerm)ATerm.getTerm(t);
		assert (pair.parts.length == 2);
		value.value.put(ATerm.getTerm(pair.parts[0]), new MutableDouble(ATerm.getDouble(pair.parts[1])));
	}

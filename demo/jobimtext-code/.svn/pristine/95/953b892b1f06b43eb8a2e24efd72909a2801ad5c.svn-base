...
//Questions about the algorithms should be directed to their authors, and not to Martin Porter (except when he is the author). 
...
public class PorterStemmer {
  ...
  //TODO: return the prefix-suffix too
  //TODO: need to combine with Collins word list for irregular words
  //TODO: 
  public String stripAffixes( String str ) {
    ...
    return str;
  	...
  	public void morphoSeparate(String surfaceWord, String[] rootPreSuf) {
  		if (rootPreSuf == null || rootPreSuf.length < 3) {
  			throw new IllegalArgumentException("The rootPreSuf argument must be an array of at least 3");
  		}
  		surfaceWord = surfaceWord.toLowerCase();
  		surfaceWord = Clean(surfaceWord);
	  
	    if (surfaceWord.length() > 2) {
	    	String noPrefix = rootPreSuf[0];
	    	noPrefix = stripPrefixes(surfaceWord);
	    	rootPreSuf[1] = surfaceWord.substring(0, surfaceWord.length() - noPrefix.length());
	    	
	    	if (noPrefix.length() > 0) {
	    		rootPreSuf[0] = stripSuffixes(noPrefix);
	    		rootPreSuf[2] = surfaceWord.substring(rootPreSuf[0].length());
	    	} else {
	    		rootPreSuf[0] = noPrefix;
	    		rootPreSuf[2] = "";
	    	}
	
	    } else {
	    	rootPreSuf[0] = surfaceWord;
	    	rootPreSuf[1] = "";
	    	rootPreSuf[2] = "";
	    }
    } 

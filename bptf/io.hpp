...
int convert_matrixmarket4(std::string base_filename, bool add_time_edges = false, bool square = false, int type = TRAINING, int matlab_time_offset = 1) {
  ...
  bool active_edge = true;
    ...
    /*
      ...
      */
      sharderobj.preprocessing_add_edge(I, J + time*M, als_edge_type(val));
      //sharderobj.preprocessing_add_edge(J + time*M, M+M+time, als_edge_type(val));
    }
...
void save_matrix_market_format_mat(const std::string datafile, const mat & output, bool issparse, std::string comment) {
  FILE * f = fopen(datafile.c_str(),"w");
  if (f == NULL)
    logstream(LOG_FATAL)<<"Failed to open file: " << datafile << " for writing. " << std::endl;
    fprintf(f, "%c%c\n%c\n%d %d\n", '%', '%', '%', (int)output.rows(), (int)output.cols());
    for (int i=0; i < output.rows(); i++) {
        for (int j=0; j < output.cols(); j++) {
            fprintf(f, "%10.13g ", output(i,j));
        }
        fprintf(f, "\n");
    }
}

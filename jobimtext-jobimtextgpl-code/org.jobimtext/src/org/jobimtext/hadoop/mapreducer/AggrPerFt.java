...
public class AggrPerFt {
	...
	public static class Map extends MapReduceBase implements
		...
		public void map(LongWritable key, Text value,
			...
			// the feature is now the value map() will outout
			// the word is now the feature map() will output
			String word = StringUtil.trim(tokens[0]);
			String feature =StringUtil.trim(tokens[1]);
	...
	public static class Reduce extends MapReduceBase implements
		...
		public void reduce(Text key, Iterator<Text> values,
			...
			// write output
			//!remove leading tab from output otherwise there will be many tabs in the output
			//especially when the reduced is called as combination of several reducers
			output.collect(key, new Text(StringUtil.trim(concat.toString())));
...
//				"org.apache.hadoop.io.compress.SnappyCodec");
		 ...
		 */
		conf.set("mapred.child.java.opts", "-Xmx10000m");

		/*
		 * how much virtual memory the entire process tree of each map/reduce
		 * task will use
		 */
		conf.set("mapred.job.map.memory.mb", "5048");
		conf.set("mapred.job.reduce.memory.mb", "5048");
